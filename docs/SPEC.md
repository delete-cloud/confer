...
`confer skill install` embeds the [canonical Skill](../skills/confer/SKILL.md) and delegates target paths, conflict protection, updates, scope, and dry-run reporting to Kitup. User scope is the default. Crates.io Kitup 0.1.4's `kimi-cli` host adapter (the kitup id for **Kimi Code**, not the legacy kimi-cli product) prefers `~/.config/agents/skills`, which Kimi Code does not scan (its user skill roots are `$KIMI_CODE_HOME/skills` and `~/.agents/skills`). That host-data fix is on kitup main (`lathe-cli/kitup#41`); Confer patches crates.io kitup to `lathe-cli/kitup` `main` so a Kimi Code user-scope install lands in `~/.agents/skills`. Existing installs already written under `~/.config/agents/skills` are not migrated. Remove the patch after the next Kitup crates.io release.
...
Cursor seats use its ACP session store. Old headless Cursor session IDs are not migrated and require new seats. Copilot seats use its native session store through ACP `session/load`; replayed history is excluded from the current answer. Kimi Code seats use native ACP `session/resume` (it also advertises `loadSession`; Confer prefers resume so history is not replayed into the current answer). Other native session stores are not rewritten. An observed session ID remains available when the prompt fails; a missing or stale ID never triggers silent replacement. A native ACP session whose model or effort configuration fails before its first prompt is not recorded, because no native work has started; the next message to that seat starts a new native session.
...
Confer launches every seat with that agent's full-permission setting so a non-interactive process is never blocked on an approval prompt it cannot answer: Claude and Antigravity receive `--dangerously-skip-permissions`; Codex receives app-server `approvalPolicy: never` and the full-access sandbox policy; Cursor receives `--trust --force`; Grok receives `--always-approve` and ACP `yoloMode`; Copilot receives `--allow-all`; and Kimi Code receives ACP `session/set_config_option` `mode=auto`. Kimi 0.41.0 maps that ACP id to engine Never Ask (`permission: "auto"`); its advertised `yolo` label is Ask When Needed and is not used. `kimi acp` has no permission flags, and even `kimi --auto acp` leaves `session/new` at `mode=default`. A Kimi Code seat remains subject to the user's static deny rules. Other seats run with the authority of the local Confer process and without sandbox isolation. Explicit task instructions remain the only limit on what a seat is asked to do.
