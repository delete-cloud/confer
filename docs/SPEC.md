...
`confer skill install` embeds the [canonical Skill](../skills/confer/SKILL.md) and delegates target paths, conflict protection, updates, scope, and dry-run reporting to Kitup. User scope is the default. Crates.io Kitup 0.1.4's `kimi-cli` host data prefers `~/.config/agents/skills`, which Kimi Code does not scan (its user skill roots are `$KIMI_CODE_HOME/skills` and `~/.agents/skills`). Confer patches Kitup to `delete-cloud/kitup` (`716a744`) so a Kimi user-scope install lands in `~/.agents/skills`. Existing installs already written under `~/.config/agents/skills` are not migrated. Remove the patch after upstream publishes that host-data fix.
